...
- **Widget accuracy at first render, fixed in the whole-branch review follow-up**: `RootView` now hydrates `WidgetSummaryCoordinator` from cached SwiftData/`ContentStore` state in its launch `.task`, by constructing throwaway `ProgressionModel`/`CheatsModel` instances (discarded after their `init` notifies the coordinator) — so the widget reflects real `overallProgress`/favorite-cheat data immediately, without requiring a prior visit to the Progression or Cheats tab. This reads only the local `ContentCacheEntry` cache (no network `syncIfNeeded()` await), so on a true first-ever launch where POI/cheat content has never synced from Firestore even once, `overallProgress` still reads 0% until that first sync happens — an accepted residual gap, since a user can't have any `FoundEntry`/`FavoriteCheat` rows to show progress for before Map/Cheats has loaded that content anyway, so the two conditions (real progress data existing, but content never cached) can't co-occur in practice.
