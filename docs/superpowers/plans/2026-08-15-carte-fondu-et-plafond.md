...
3. **La spec dit « au repos, la carte remplit l'écran : le fondu est hors champ ». C'est faux, et le plan le corrige.** L'échelle de repos couvre l'écran sans bande vide, donc la carte affleure EXACTEMENT deux des quatre bords : sur iPad, 2 048 × 0,671875 = 1 376 pt pour une fenêtre de 1 376 pt. Un fondu toujours actif poserait donc une vignette de 80 pt en haut et en bas de l'écran au repos — précisément ce que la section « Ce qu'on ne fait pas » interdit (« Pas de fondu au repos »). L'opacité est donc conditionnelle.

   **Corrigé après la relecture de la tâche 4, arbitré par le commanditaire le 2026-08-15 — le reste de ce plan décrit encore l'ancienne règle, et c'est CELLE-CI qui gouverne.** La première rédaction réglait l'opacité sur la distance dont le bord le plus proche avait quitté l'écran, `(contentSize × zoom − bounds) / 2` — ce qui suppose la carte CENTRÉE. Elle ne l'est pas : `centeringInsets` plancher les encarts à une demi-fenêtre dès qu'on dépasse le repos, pour qu'une épingle côtière puisse être amenée au centre. Entre z = 0,672 et z = 0,75 sur iPad (0,427 à 0,505 sur iPhone), on pouvait donc tirer un bord de carte jusqu'au milieu de l'écran avec un fondu à 10 % : l'arête franche que ce chantier existe pour supprimer, dans la bande de zoom où l'on passe le plus de temps.

   La règle qui gouverne : **l'opacité se règle sur le FOND visible à côté de la carte**, pas sur la taille de celle-ci. `edgeFade` reçoit le rectangle visible en coordonnées de contenu — que `sync` calcule déjà — et prend le maximum du débord sur les quatre côtés, un seul calque les servant tous. Nulle au repos comme avant ; nulle aussi à carte centrée quel que soit le zoom, donc strictement moins de vignette qu'avant ; entière dès qu'un bord entre dans la fenêtre. La spec avait raison sur la règle et tort sur le motif ; la première rédaction du plan avait raison sur le motif et tort sur la mesure.
