...
struct POIClustererTests {
    ...
    @Test func poisWithoutPositionAreExcluded() {
        ...
        let clusters = MapClusterer.clusters(items: [placed, unplaced], zoomScale: 1, contentSize: 2048)
    ...
    @Test func nearbyPOIsAggregateAndDistantOnesDoNot() {
        ...
        let clusters = MapClusterer.clusters(
            items: [poi("a", 0.20, 0.20), poi("b", 0.201, 0.201), poi("far", 0.90, 0.90)],
    ...
    @Test func zoomingInSplitsAggregates() {
        ...
        let farOut = MapClusterer.clusters(items: pois, zoomScale: 0.4, contentSize: 2048)
        let closeIn = MapClusterer.clusters(items: pois, zoomScale: 2.5, contentSize: 2048)
    ...
    @Test func nothingStaysAggregatedPastTheDisaggregationZoom() {
        ...
        let clustered = MapClusterer.clusters(items: pois, zoomScale: 1.5, contentSize: 2048)
        #expect(clustered.count == 1, "à zoom moyen ils doivent encore être groupés")

        let split = MapClusterer.clusters(
            items: pois, zoomScale: MapClusterer.disaggregationZoom, contentSize: 2048
        )
    ...
    @Test func disaggregatedPinsKeepTheirOwnIdentityAndCategory() {
        ...
        let split = MapClusterer.clusters(items: pois, zoomScale: 2.5, contentSize: 2048)
    ...
    @Test func aggregatePositionIsTheCentroidOfItsMembers() {
        let clusters = MapClusterer.clusters(
            items: [poi("a", 0.300, 0.400), poi("b", 0.302, 0.404)], zoomScale: 0.3, contentSize: 2048
        )
        #expect(clusters.count == 1)
    ...
    @Test func aggregateTakesTheColorOfItsMostFrequentCategory() {
        let clusters = MapClusterer.clusters(
            items: [poi("a", 0.5, 0.5, .collectible),
    ...
    @Test func clusteringIsStableBetweenQuantizationSteps() {
        ...
        let a = MapClusterer.clusters(items: pois, zoomScale: 1.00, contentSize: 2048)
        let b = MapClusterer.clusters(items: pois, zoomScale: 1.05, contentSize: 2048)
        #expect(MapClusterer.level(for: 1.00) == MapClusterer.level(for: 1.05))
    ...
    @Test func outputOrderIsDeterministic() {
        ...
        let runs = (0..<5).map { _ in MapClusterer.clusters(items: pois, zoomScale: 0.5, contentSize: 2048).map(\.id) }
    ...
    @Test func everyPOIEndsUpInExactlyOneAggregate() {
        ...
        let clusters = MapClusterer.clusters(items: pois, zoomScale: 0.45, contentSize: 2048)
    ...
    @Test func degenerateInputsDoNotCrash() {
        #expect(MapClusterer.clusters(items: [POI](), zoomScale: 1, contentSize: 2048).isEmpty)
        #expect(MapClusterer.clusters(items: [poi("a", 0.5, 0.5)], zoomScale: 0, contentSize: 2048).isEmpty == false)
        #expect(MapClusterer.clusters(items: [poi("a", 0.5, 0.5)], zoomScale: 1, contentSize: 0).isEmpty)
    }
}
